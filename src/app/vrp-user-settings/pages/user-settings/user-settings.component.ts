...
export class VrpUserSettingsComponent {
    ...
    constructor(
        ...
        try {
            ...
            if (this.user.role === 'default' || this.user.role === 'admin') {
                this.customDataTemplates = [
                    { label: 'Vehicle', icon: 'local_shipping', content: () => _get(this.user, 'data.vehicles.length', 0) + '  vehicle(s)' },
                    { label: 'Vehicle Types', icon: 'removed_circle', content: () => _get(this.user, 'data.vehicle_types.length', 0) + ' vehicle type(s)' },
                    { label: 'Items', icon: 'list', content: () => _get(this.user, 'data.items.length', 0) + ' item(s)' },
                ];
            }
        } catch (err) {
