...
export class PlannerDispatchComponent implements OnInit, OnChanges {
    ...
    private _moveJobsUpOrDown(from: any, movedJobs: any[], moveUp: boolean): boolean {
        ...
        if (PlannerUtils.containFinishedJob(movedJobs)) {
            this._dialog.error(`Selection contains FINISHED jobs. Only unfinished jobs are allowed to be moved.`);
            return true;
        }
        ...
        if (!this._isEnoughRoomToMove(from.data, movedJobs, moveUp)) {
            this._dialog.error(`Reach the limit. Unable to move jobs.`);
            return true;
        }

        if (!this._isShipmentConstraintSatisfy(from.data, movedJobs, moveUp)) {
            this._dialog.error(`Unable to place pickup job after delivery job in shipment order`);
            return true;
        }
    ...
    private _moveJobsLeftorRight(from: any, to: any, selectedJobs: any[]): boolean {
        ...
        this._sortJobArraysBasedOnIndex(movedJobs, from.data);
        if (PlannerUtils.containFinishedJob(movedJobs)) {
            this._dialog.alert(`Selection contains FINISHED jobs. Only unfinished jobs are allowed to be moved.`);
            return true;
        }
    ...
    private _notifyChanges() {
        ...
        this._plannerData.onPlanChanged.next({ routes, unassignedOrderIds });  // inform planner-monitor that plan has changed
    }
