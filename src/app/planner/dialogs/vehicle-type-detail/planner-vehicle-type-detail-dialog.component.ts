...
export class PlannerVehicleTypeDetailDialogComponent {
    ...
    constructor(
        ...
        this.f = new FormGroup({
            Name: new FormControl('TypeA', [Validators.required, Validators.pattern(ID_REGEX_PATTERN), this.checkIdExistence(data.allVehicleTypeNames.map((i) => i.toUpperCase()))]),
            Capacity: new FormControl(1000, []),
            FixedCost: new FormControl(100, []),
            DistanceCost: new FormControl(0.0001, []),
            TravelTimeCost: new FormControl(0.0017, []),
            WaitingTimeCost: new FormControl(0.0017, []),
        });
