...
export class PlannerDeliveryDetailComponent implements OnInit, OnDestroy {
    ...
    static createForm(val): FormGroup {
        ...
        const newForm = new FormGroup({
            ...
            ContactEmail: new FormControl(undefined, [Validators.pattern(EMAIL_REGEX_PATTERN)]),
            JobType: new FormControl('PICKUP', []),
            JobSequence: new FormControl(0, []),
            ServiceTime: new FormControl(10, [Validators.required]),
            Address: new FormControl(undefined, []),
            Postal: new FormControl('', [Validators.required, Validators.minLength(6), Validators.pattern(POSTAL_REGEX_PATTERN)], // sync validators
                // [VrpValidators.postalExistValidator(this._geocode, (res) => this._setCoords(res))            ]
            ), // async validators
            Lat: new FormControl(undefined, [Validators.required, VrpValidators.LatInSingapore]),
            Lng: new FormControl(undefined, [Validators.required, VrpValidators.LngInSingapore]),
            StartTimeWindow: _startTimeWindow,
            EndTimeWindow: new FormControl(undefined, [Validators.required, VrpValidators.endTimeWindowValidator(_startTimeWindow), VrpValidators.timeWindowValidator(_startTimeWindow)]),
            VerificationCode: new FormControl(undefined, []),
            NoteFromPlanner: new FormControl([], []),
            DeliveryItems: new FormControl([], [VrpValidators.arrayNotEmpty()]),
